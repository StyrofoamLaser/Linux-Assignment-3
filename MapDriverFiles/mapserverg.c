...
{
	if (msgValidity == 0 && srcMsg.msgType == 'M') /* Send a default map message */
	{
	...
	else if (msgValidity == 1 && srcMsg.msgType == 'M') /* Send a custom map message */
	{
	...
	else if(msgValidity == 0 && srcMsg.msgType == 'K') /* If the same character exists at the indicated point remove it */
	{
		int arrayPos = srcMsg.param + (srcMsg.param2 * curWidth + 1);		
		char readChar[1];

		lseek(connfd, 0, arrayPos);
		read(connfd, readChar, 1);
		/*If char is the same as the selected char change to a space */
		if(readChar[0] == srcMsg.map[0])
		{
			char writeChar[1];			
			writeChar[0] = ' ';
			lseek(connfd, 0, arrayPos);
...
int interpretMsg(mapmsg_t msg)
{
	...
	else if(msg.msgType == 'K')
	{		
		if(msg.param < curWidth && msg.param2 < (curHeight-1) )
		{
